 ...
 *   - CPU + Req/s: {@link LineChart} with CPU (yMin/yMax fixed, left) + Req/s (dual Y right)
 *   - Requests: {@link AreaChart} with User + Sys stacked cumulative area
...
import { randomWalkGen, noisySineGen, spikyGen, type Generator } from './generators.ts';
...
export const PANELS: PanelDef[] = [
  {
    id: 'cpu',
    title: 'CPU + Req/s (dual Y)',
    chartType: 'line',
    windowMul: 1,
    batchMul: 1,
    series: [
      { config: { name: 'CPU', color: '#4ea8ff', lineWidth: 1.6, yMin: 0, yMax: 100 }, gen: () => randomWalkGen() },
      {
        config: { name: 'Req/s', color: '#ffb454', lineWidth: 1.2, dash: [8, 4], yAxis: 'right' },
        gen: () => noisySineGen(),
      },
    ],
    kpi: { label: 'CPU', unit: '%' },
...
export function buildPanel(gridHost: HTMLElement, kpiHost: HTMLElement, def: PanelDef, windowSize: number): Panel {
